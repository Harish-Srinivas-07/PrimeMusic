document.addEventListener("DOMContentLoaded", function () {
  ...
  const randomButton = document.querySelector(
    ".song-control__buttons .fa-random"
  );
  const forwardButton = document.querySelector(
    ".song-control__buttons .fa-forward"
  );
  const stepBackwardButton = document.querySelector(
    ".song-control__buttons .fa-step-backward"
  );
  const redoButton = document.querySelector(
    ".song-control__buttons .fa-redo-alt"
  );
  ...
  function updateCurrentSongDetails(songElement) {
    ...
    const songSrc = songElement.querySelector(
  ...
  document
    .getElementById("VelaiIllaPadathari")
  ...
  // jailer playlist
  fetch("https://saavn.dev/api/search/songs?query=tamil+2023&page=1&limit=80")
    .then((response) => response.json())
    .then((data) => {
      populateBoxItems(data.data.results);
    })
  ...
  function populateBoxItems(songs) {
    ...
    songs.forEach((song) => {
      ...
      boxItem.addEventListener("click", function () {
        const songSrc = this.querySelector(
          'p[style="display: none;"]'
        ).textContent;
        audioPlayer.src = songSrc;
        const mobplayButton = document.querySelector(".play-button-circled");
        ...
        document.querySelector(".song-control__details").style.display =
          "block";
        ...
        document
          ...
          .addEventListener("click", () => {
            handleDownload(
              document.getElementById("currentSongImg"),
        ...
        document
          .getElementById("mobcurrentSongImg")
          .addEventListener("click", () => {
            handleDownload(
              document.getElementById("mobcurrentSongImg"),
              document.getElementById("mobcurrentSongName")
            );
            mobcurrentSongImg.removeEventListener(
              "click",
              mobcurrentSongListener
            );
          });
        ...
        const handleDownload = async (imgElement, nameElement) => {
          ...
          if (songName && audioPlayer.src) {
            ...
            console.error("No audio source to download.");
          }
